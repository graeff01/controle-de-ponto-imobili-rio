...
class ReportsService {
  ...
  async getDashboardStats() {
    try {
      // 1. Stats de funcionários CLT (usando data BRT)
      const employeeStats = await db.query(`
        SELECT
          COUNT(*) FILTER (WHERE u.is_duty_shift_only = false) as total_clt,
          COUNT(DISTINCT tr.user_id) FILTER (WHERE u.is_duty_shift_only = false AND DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date AND tr.record_type = 'entrada') as presentes_clt,
          COUNT(DISTINCT tr.user_id) FILTER (WHERE u.is_duty_shift_only = false AND DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date AND tr.record_type = 'saida_final') as concluidos_clt
        FROM users u
        LEFT JOIN time_records tr ON u.id = tr.user_id AND DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date
      ...
      const brokerStats = await db.query(`
        SELECT 
          COUNT(*) as total_pj,
          (SELECT COUNT(DISTINCT user_id) FROM duty_shifts WHERE date = (NOW() AT TIME ZONE 'America/Sao_Paulo')::date) as presentes_pj
        FROM users 
        WHERE status = 'ativo' AND is_duty_shift_only = true
      `);

      // 3. Inconsistências (Últimos 7 dias, usando data BRT)
      const inconsistencias = await db.query(`
        SELECT COUNT(*) as total
        FROM (
          SELECT user_id, DATE(timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') as data
          FROM time_records
          WHERE DATE(timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') >= (NOW() AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '7 days'
          GROUP BY user_id, DATE(timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')
          HAVING
      ...
      // 5. Jornadas incompletas (entrada sem saída nos últimos 3 dias úteis, data BRT)
      const jornadasIncompletas = await db.query(`
        SELECT DISTINCT u.id, u.nome, u.matricula,
          DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') as data,
          MAX(tr.record_type) as ultimo_tipo
        FROM time_records tr
        JOIN users u ON tr.user_id = u.id
        WHERE DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') >= (NOW() AT TIME ZONE 'America/Sao_Paulo')::date - INTERVAL '3 days'
          AND DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') < (NOW() AT TIME ZONE 'America/Sao_Paulo')::date
          AND u.is_duty_shift_only = false
          AND u.status = 'ativo'
        GROUP BY u.id, u.nome, u.matricula, DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')
    ...
    try {
      ...
      let query = `
        SELECT
          ...
          COUNT(DISTINCT DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')) as dias_trabalhados,
          COALESCE(SUM(hwd.hours_worked), 0) as total_horas
        FROM users u
        LEFT JOIN time_records tr ON u.id = tr.user_id
          AND DATE(tr.timestamp AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo') BETWEEN $1 AND $2
